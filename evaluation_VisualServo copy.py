...
class rendar():
    def __init__(self, num_envs = 2):
        ...
        # targetG_weight = torch.load("src/network/log/all_0823/weight/G_delete/weights_last.pth", map_location='cuda')
        # deleteG_weight = torch.load("src/network/log/all_0823/weight/G_target/weights_last.pth", map_location='cuda')
        ...
        # action_weight = torch.load("src/network/log/all_0823/weight/action/weights_last.pth", map_location='cuda')
        ...
        # self.generate_target.load_state_dict(targetG_weight)
        # self.delete_ur.load_state_dict(deleteG_weight)
        ...
        self.ur_default_dof_pos = torch.tensor([0., 0, 1., 0, 0, 0])
        self.ur_default_dof_pos = torch.tensor([1.57, 0, 0., 0, 0, -1.57])
        # self.ur_default_dof_pos = torch.tensor([1.57, 0, 0., 0, 0, -1.57, 0])
    ...
    def create_envs(self, i = 1, spacing = 10, num_per_row = 10):
        ...
        self.cam_pos = gymapi.Vec3(0.65, 0, 1.)
        self.cam_target = gymapi.Vec3(-0.8, 0, 1.0)
        ...
        ur_asset_file = "urdf/ur3/robots/ur3_hand_to_hook.urdf"
        # ur_asset_file = "urdf/universal-robot-ur5/ur5robot.urdf"
        closet_asset_file = "urdf/closet_for_sdf/urdf_varius_size/10_896_944.urdf"
        closet_asset_file = "urdf/closet_for_sdf/closet_various_handleposition/closet_small.urdf"
        ...
        asset_box = self.gym.create_box(self.sim, 0.8, 100.0, 4, asset_options)
        asset_camera = self.gym.create_box(self.sim, 0.15, 0.08, 0.08, asset_options)
        asset_camera = self.gym.create_capsule(self.sim, 0.03, 0.04, asset_options)
        ...
        self.num_closet_bodies = self.gym.get_asset_rigid_body_count(closet_asset)
        self.num_closet_dofs = self.gym.get_asset_dof_count(closet_asset)
        ...
        closet_dof_props = self.gym.get_asset_dof_properties(closet_asset)
        ...
        for i in range(self.test_num_envs):
            ...
            self.closet_actor = self.gym.create_actor(self.env_ptr, closet_asset, closet_pose, "closet", i, 0, 0)
            self.gym.set_actor_dof_properties(self.env_ptr, self.closet_actor, closet_dof_props)

            pose = gymapi.Transform()
            pose.p = gymapi.Vec3(-1.4, -49, 1.25)
            ...
            pose_camera.p = gymapi.Vec3(0.275, -0.1, 6.6)
            # pose.p = gymapi.Vec3(-1.4, 0, 1.25)
            ...
            self.gym.set_rigid_body_color(self.env_ptr, camera_handle, 0, gymapi.MESH_VISUAL, gymapi.Vec3(0.4, 0.4, 0.4))
            ###################
    ...
    def ini(self):
        ...
        # self.root_tensor[:, 3, :3] = torch.tensor([0.375, -0.1, 1.])
        ...
        self.gym.refresh_dof_state_tensor(self.sim)
        now = time.time() 
        self.gym.write_viewer_image_to_file(self.viewer, f"/home/engawa/py_ws/visual_servo/src/network/images_sice/result/env.png")
    ...
    def visual_servo(self, angle, i):
        ...
        # handle_x = (handle_trajectory[:, 0] - 0.01) - hand_x
        ...
        hand_x = self.ur_dof_state[:self.test_num_envs, 1, 0].clone()

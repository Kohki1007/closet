...
class rendar():
    def __init__(self, num_envs = 5):
        ...
        self.closet_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/real_sameclosetsize/closet_posistion.pt")
        # self.closet_rot = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/real_sameclosetsize/closet_rot.pt")
        self.wall_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/real_sameclosetsize/wall_position.pt")
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        handle_prop_list = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/real_sameclosetsize/closet_size.pt")
    ...
    def pre_physics_step(self, step, position):
        ...
        target_angle = torch.tensor([step, step, step, step, step])
        # target_angle += 10
    ...
    def pre_physics_action(self, step, position):
        ...
        if step  == 180 and position == 1:
            print(torch.any(torch.isnan(self.action)))
            torch.save(self.action, f"/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/real_sameclosetsize/trajectory.pth")



            action = torch.zeros(self.num_envs, 2, 180, 6)
                ...
                for position in range(action.shape[1]):
                    for step in range(action.shape[2]):
                        if step < 176:
                            ...
                            action[env, position, step, :6] = self.action[env, position, step + 4] - self.action[env, position, step]
                        else:
                            action[env, position, step, :6] = self.action[env, position, -1] - self.action[env, position, step]
            ...
            torch.save(action, "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/real_sameclosetsize/action.pth")
    ...
    def get_image(self, step, position):
        ...
        if position == 1 and step == 180:
            # print("yes")
            SAMPLE_DIR = "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/real_sameclosetsize"

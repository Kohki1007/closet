...
class rendar():
    def __init__(self, num_envs = 40):
        ...
        self.closet_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/camera_pose_random/closet_posistion.pt")
        self.closet_rot = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/camera_pose_random/closet_rot.pt")
        self.wall_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/camera_pose_random/wall_position.pt")
        self.camera_pose = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/camera_pose_random/camera_pose.pt")
        ...
        self.camera_rot_left = []

        self.repeat = 0
        self.number = 100 + self.repeat * self.num_envs 
        self.number = 0

        self.camera_transoformlist = []
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        handle_prop_list = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/camera_pose_random/closet_size.pt")
        ...
        for i in range(self.num_envs):
            ...
            # print(self.closet_position[asset_id + self.number, 0, 1])
            ...
            self.camera_transoformlist.append(gymapi.Transform())
            self.camera_transoformlist[i].p = gymapi.Vec3(0.3,-0.1,1)
            self.camera_transoformlist[i].r = gymapi.Quat(0, 0, 0, 1)
            ...
            target_y = (math.tan(self.camera_pose[i + self.number]*3.14/180) * -0.7) - 0.1
            # print(target_y)
            # print(self.camera_pose[i + self.number], self.camera_pose[i + self.number]*3.14/180)
            self.camera_rot_left = gymapi.Vec3(-0.4, target_y, 1)
    ...
    def pre_physics_action(self, step, position):
        ...
        if step  == 180 and position == 4:
            ...
            torch.save(self.action, f"/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/camera_pose_random/trajectory_{self.repeat}.pth")

            action = torch.zeros(self.num_envs, 5, 40, 6)

            for env in range(action.shape[0]):
                for position in range(action.shape[1]):
            ...
            torch.save(action, "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/camera_pose_random/action.pth")
    ...
    def get_image(self, step, position):
        ...
        for l in range(self.num_envs):
            ...
            color_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/test_camera_pose/{l}_{position}_{step}.png"
        ...
        #     # print("yes")
        #     SAMPLE_DIR = "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/camera_pose_random"
        ...
        #     test_input_name = f"/current_withhandle_normal.pt"
        #     test_input_path = SAMPLE_DIR + test_input_name
        #     target_input_name = f"/target_withhandle_normal.pt"

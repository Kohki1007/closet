...
from model import ivis, manipulator_remover
...
class train_function():
    def __init__(self, train_subject = "goal_image_generator", 
        ...
        if self.train_subject == "goal_image_generator":
            self.optimizer = optim.AdamW(           
                self.model.goal_image_generator.parameters(),
    ...
    def forward(self, image1, image2 = None):
        ...
        if self.train_subject == "goal_image_generator":
            self.x = self.model.goal_image_generator(image1)
        elif self.train_subject == "manipulator_remover":
            self.x = self.model(image1)
        elif self.train_subject == "action_generator":
            self.x = self.model.action_generator(image1, image2)
        else:
    ...
    def rename_key(self, d):
        ...
        for key in keys_list:
            # new_key = key.replace('goal_image_generator.encoder.', '')
    ...
    def delete_key(self, weight):
        ...
        keys_to_delete = [key for key in weight.keys() if 'action_generator' in key]
        for key in keys_to_delete:
            del weight[key]
    ...
    def train_step_remover(self, input1, output, epoch, input2 = None):
        if epoch > 40:
            ...
            input1 = self.add_noise(input1, noiz_level)
        input1 = input1.repeat([1, 3, 1, 1,])
        self.model.train()
        self.optimizer.zero_grad()
        self.forward(input1, input2)
    ...
    def save_weight(self, path, epoch, last = None, best = None):
        if last :
            save_path = "log/" + path + f'/weight/weights_last.pth'
        elif best:
            save_path = "log/" + path + '/weight/weights_best.pth'
        else:
            save_path = "log/" + path + f'/weight/weights{epoch}.pth'
    ...
    def plot_grad(self, epoch):
        for name, means in self.gradient_means.items():
            ...
            plt.legend()
            plt.savefig(f'check/{name}_{epoch}.png')
            plt.clf()
    ...
    def forward_solo(self, input):
        ...
        for i in range(self.x.shape[0]):
            ...
            depth = Image.fromarray((depth).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth.save(f"image/eval/goal_image/input/{i}.png")
            depth_predict = self.x[i, 0].to('cpu').detach().numpy().copy()
            depth_predict *= 255
            print(depth_predict.shape)
            depth_predict = Image.fromarray((depth_predict).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth_predict.save(f"image/eval/goal_image/predict/{i}.png")
            # depth_predict = target[i, 0].to('cpu').detach().numpy().copy()
            # depth_predict *= 255
            # depth_predict = Image.fromarray((depth_predict).astype(np.uint8))
            # # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            # depth_predict.save(f"image/eval/remover/output/{i}.png")
    ...
    def add_noise_shapes(self, image):
        ...
        for i in range(shape_number):
            ...
            depth.save(f"/home/kokimoriya/ur5e_handeye_ws-main/src/door/scripts/image/generator/noiz_all_{i}.png")

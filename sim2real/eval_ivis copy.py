...
class rendar():
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        self.current_position = torch.zeros([self.num_envs, 3]).to("cuda")
        self.current_orientation = torch.zeros([self.num_envs, 4]).to("cuda")

        for j in range(self.num_envs):
            self.current_position[j] = self.rigid_body_states[sum(self.object_count_list[:j]) + 9, 0:3].to("cuda")
            self.current_orientation[j] = self.rigid_body_states[sum(self.object_count_list[:j]) + 9, 3:7].to("cuda")
...
##########################################################3####変える
    ...
    def step_ur(self, current_position, goal_position, i):
        ...
        while difference > 0.001:
            ...
            d_dof = self.ik(ja, self.current_position, self.current_orientation, goal_position).to("cpu")
            ...
            difference = torch.sqrt(torch.sum((self.current_position-goal_position)**2))
            # print(difference)
    ...
    def opening(self, angle, i):
        ...
        goal_position = self.current_position[:, :2].to("cpu") + self.action[:2].to("cpu")
        ...
        difference = torch.sqrt(torch.sum((self.current_position[:, :2].to("cpu")-goal_position)**2))
        goal_position =  goal_position.to("cuda")
        ...
        if torch.isnan(self.current_position).any().item():
            for _ in range(10000):
                self.pre_physics_step_brank()
        # dd = 
        ...
        d = (self.action[:2] / 5).to("cuda").clone()

        for i in range(5):
            if i == 0:
                goal_position = self.current_position.clone()
            goal_position[:, :2] += d
    ...
    def get_image(self):
        self.gym.render_all_camera_sensors(self.sim)
        image = torch.zeros([self.num_envs, 1, 224, 224])
        for l in range(self.num_envs):
            ...
            image[l] = color_image_left.clone()
...
if __name__ == "__main__": 
    ...
    for position in range(5):
        ...
        while step:
            rend.generate_current_image()
            rend.generate_action()
            step, target_angle = rend.opening(target_angle, i)
            i += 1
...
class rendar():
    def __init__(self, num_envs = 1):
        ...
        self.ur_default_dof_pos = self.to_torch([3.6, -1.2, 0.8, -2.4, -1.57, 0], device=self.device)
        ...
        # print("good")
        # self.number = 0
        # self.current_image = torch.zeros([self.num_envs, 15, 60, 224, 224])
        # self.target_image = torch.zeros([self.num_envs, 15, 60, 224, 224])
        ...
        self.input = torch.zeros([1, 224, 224])
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        # print(asset_root)
        # asset_root = "../../assets"
        ur_asset_file = "urdf/ur3/robots/ur3_hook.urdf"
        ...
        self.camera_props1.horizontal_fov = 80.797
    ...
    def pre_physics_action(self, step):
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self.gym.refresh_dof_state_tensor(self.sim)
        self.gym.refresh_rigid_body_state_tensor(self.sim)
        self.gym.refresh_jacobian_tensors(self.sim)
        ...
        #     torch.save(self.action, f"/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/action/handle_trajectory_{self.repeat}.pth")
    ...
    def get_image(self, step):
        ...
        for l in range(self.num_envs):
            ...
            depth_image /= torch.max(depth_image)/255
            depth_image = self.transform(depth_image)
            # print(depth_image)
            depth_image = depth_image.squeeze().to('cpu').detach().numpy().copy()
            ...
            # else:
            self.input = self.transform(depth_image).clone().squeeze()
            torch.save(self.input, "/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/goal_input.png")

            color_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/predict_goal_{step}.png"
            depth_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/predict_goal_depth_{step}.png"
        ...
        # if position == 14 and step == 60:
        #     # print("yes")
        #     SAMPLE_DIR = "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/dataset_action"
        ...
        #     test_input_name = f"/current_action.pt"
        #     test_input_path = SAMPLE_DIR + test_input_name
        #     target_input_name = f"/target_action.pt"

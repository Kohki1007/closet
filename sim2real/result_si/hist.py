...
for filename in glob.glob(file_pattern):
    ...
    if tensor[0] != 0:
        result_list.append(torch.load(f"{filename}")/2)
    if tensor[0] > 70:
        i += 1
...
x = torch.stack(result_list).to('cpu').detach().numpy().copy()
...
plt.savefig('/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/hist.png')
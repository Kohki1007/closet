...
class rendar():
    def __init__(self, num_envs = 5):
        ...
        self.ur_default_dof_pos = self.to_torch([0, 0, 0.8, 0, 0, 0], device=self.device)
        ...
        self.closet_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/remover_sim/closet_posistion.pt")
        self.closet_rot = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/remover_sim/closet_rot.pt")
        ...
        # print("good")
        # self.number = 0
        self.current_image = torch.zeros([100, 10, 40, 224, 224])
        # self.target_image = torch.zeros([100, 10, 40, 256, 256])
        # self.current_image = torch.zeros([20, 5, 40, 256, 256])
        # self.action = torch.zeros([100, 15, 181, 6])
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        handle_prop_list = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/remover_sim/closet_size.pt")
        ...
        ur_start_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(1,0, 0), np.radians(180))
    ...
    def pre_physics_action(self, step, position):
        ...
        self.action[:, position, step, :3] = self.rigid_body_states[:, self.closet_handle][:, :3].clone()
        # print()
        ...
        if step  == 180 and position == 14:
            print(torch.any(torch.isnan(self.action)))
            # torch.save(self.action, "/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props_action/handle_trajectory.pth")
    ...
    def get_image(self, step, position):
        ...
        if position == 9 and step == 39:
            ...
            test_input_path = SAMPLE_DIR + test_input_name
            target_input_name = f"/target_all.pt"

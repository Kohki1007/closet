...
class rendar():
    def __init__(self, num_envs = 1):
        ...
        self.closet_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/closet_posistion_object.pt")
        # self.closet_rot = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/closet_rot_object.pt")
        # self.wall_position = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/wall_position_object.pt")
        handle_trajectory = torch.load("/home/engawa/py_ws/visual_servo/src/network/dataset/dataset_real/dataset_base/handle_trajectory.pth")
        closet_rot = torch.load("/home/engawa/py_ws/visual_servo/src/network/dataset/dataset_real/closet_props/closet_rot.pt")

        handle_trajectory = handle_trajectory[1]
        action = torch.zeros([5, 60, 6])

        self.closet_rot = torch.zeros([5, 61])
        self.closet_rot = closet_rot[1]

        # self.closet_rot[:, 60] = 180

        for position in range(action.shape[0]):
            for step in range(action.shape[1]):
                start_point = self.closet_rot[position, step]
                print()

                action[position, step, :6] = handle_trajectory[position, int(start_point) + 4] - handle_trajectory[position, int(start_point)]
                # output[step, 6:] = handle_trajectory[int(start_point)]
                action[position, step, 2] = 0

                action[position, step, :3] /= torch.sqrt(torch.sum(action[position, step, :3]**2))
                action[position, step, 3:] /= torch.sqrt(torch.sum(action[position, step, 3:]**2))
        
        
        torch.save(self.closet_rot, "/home/engawa/py_ws/visual_servo/src/network/sim2real/validation_dataset_actiongenerator/closet_rotation.pt")
        torch.save(action, "/home/engawa/py_ws/visual_servo/src/network/sim2real/validation_dataset_actiongenerator/action.pt")
        ...
        self.repeat = 0
        self.number = self.repeat * 5 
        # self.number = 0

        self.current_image = torch.zeros([5, 60, 256, 256])
        self.target_image = torch.zeros([5, 60, 256, 256])
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 1):
        ...
        handle_prop_list = torch.load("/home/engawa/py_ws/visual_servo/src/network/sim2real/closet_props/closet_size_object.pt")
        ...
        for a in range(self.num_envs):
            ...
            edit_urdf_handle_position(scale_id, a, 0.9)
            ...
            file_path = f'urdf/closet_for_real/urdf_real_structure/{a}.urdf'
            self.asset_path_list.append(file_path)
    ...
    def set_closet_position(self, i):
        ...
        self.root_tensor[:, 1, :2] = self.closet_position[1, i]
    ...
    def pre_physics_step(self, step, position):
        ...
        # print(self.closet_rot[0, 0])
        # print(self.closet_rot)
        # print(position)
        # print(step)
        # target_angle += 10
    ...
    def get_image(self, step, position):
        ...
        for l in range(self.num_envs):
            ...
            if step == 60:
                ...
                self.target_image[position] = self.transform(depth_image).clone()
            else:
                # print(self.transform(depth_image))
                self.current_image[position, step] = self.transform(depth_image).clone().squeeze()

            color_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/current_ob/{l}_{position}_{step}.png"
        ...
        if position == 4 and step == 60:
            # print("yes")
            SAMPLE_DIR = "/home/engawa/py_ws/visual_servo/src/network/sim2real/validation_dataset_actiongenerator"
            ...
            # print(self.current_image.shape[2])
            for j in range(self.current_image.shape[0]):
                for k in range(self.current_image.shape[1]):
                    # print(k)
                    self.current_image[j, k] /= torch.max(self.current_image[j, k])
                    self.target_image[j, k] /= torch.max(self.target_image[j, k])
                    
                # self.depth_closet_target[i, j, 0] /= torch.max(self.depth_closet_target[i, j, 0])

            test_input_name = f"/current_image.pt"
            test_input_path = SAMPLE_DIR + test_input_name
            test_output_name = f"/target_image.pt"
            test_output_path = SAMPLE_DIR + test_output_name

            
            torch.save(self.current_image, test_input_path)
            torch.save(self.target_image, test_output_path)
            print(self.repeat)
            print("complete")

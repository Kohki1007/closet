...
class rendar():
    def __init__(self, num_envs = 4):
        ...
        # print("good")
        # self.number = 0
        self.current_image = torch.zeros([self.num_envs, 2, 181, 224, 224])
        self.target_image = torch.zeros([self.num_envs, 2, 181, 224, 224])
        # self.current_image = torch.zeros([20, 5, 40, 256, 256])
        self.action = torch.zeros([self.num_envs, 2, 181, 6])

        self.repeat = 1
        self.number = 100 + self.repeat * self.num_envs 
        self.number = 0

        self.scale_list = [0.8, 1.0, 1.2, 1.4, 1.6]

        print(self.closet_position.shape)
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 1):
        ...
        id = random.randint(1, self.closet_position.shape[0]//4)
        self.closet_position_list = self.closet_position[id + 1]
        # print(self.closet_position.shape) 守屋こうきよ、永遠に

        size_list = [0.8, 1.0, 1.2, 1.4, 1.6]
        ...
        for a in range(self.num_envs):
            ...
            edit_urdf_handle_position(self.scale_list[a], a, 1.0)
        ...
        # position_id_list = random.sample(range(1, 40), 4)
        # for k in range(len(position_id_list)):
        #     closet_x.append(self.closet_position[position_id_list[k] * 4 + k, 0, 0] + 0.1)
        #     closet_y.append(self.closet_position[position_id_list[k] * 4 + k, 0, 1])
        ...
        for i in range(self.num_envs):
            ...
            # closet_pose.p.x = self.closet_position_list[i, 0]
            # closet_pose.p.y = self.closet_position_list[i, 1]
            closet_pose.p.z = 0

            closet_pose.p.x = self.closet_position[i, 0]
            closet_pose.p.y = self.closet_position[i, 1]
            print(self.closet_position[i])
            # closet_pose.p.z = 0
    ...
    def define_props(self):
        self.closet_position = torch.zeros(self.num_envs, 2)
        self.closet_rot = torch.zeros(self.num_envs, 5)
        ...
        for i in range(self.closet_position.shape[0]):
            ...
            self.closet_position[i, 0] = random.uniform(wall_position[i] + 1., wall_position[i] + 1.15)

            dy_max = (y_max[id+1] + 0.1) * ((self.closet_position[i, 0] + 0.3)/-0.95)
            print(dy_max)
            dy_min = (y_min[id+1] + 0.1) * ((self.closet_position[i, 0]+0.3)/-0.95)
            print(dy_min)


            self.closet_position[i, 1] = random.uniform(dy_min - 0.1, dy_max - 0.1)
            self.closet_position[i, 1] = random.uniform(dy_min - 0.2, dy_max + 0.1)

            self.closet_rot[i, :-1] = torch.tensor(sorted(random.sample(range(170), k=4)))
            self.closet_rot[i, -1] = 180
    ...
    def set_closet_position(self, position):
        ...
        self.root_tensor[:, 1, :2] = self.closet_position_list[:, position]
        if position == 0:
            self.root_tensor[:, 2, 0] = -1.6
        elif position == 1:
            self.root_tensor[:, 2, 0] = -1.8
    ...
    def pre_physics_step(self, step, position):
        ...
        target_angle = torch.tensor([step, step, step, step])
        target_angle = self.closet_rot[:, step]
        # target_angle += 10
    ...
    def get_image(self, step, position):
        ...
        for l in range(self.num_envs):
            ...
            depth_image = color_image_left / torch.max(color_image_left) * 255
            depth_image = self.transform(depth_image.unsqueeze(0))
            # depth_image = depth_image.unsqueeze(0)

            depth = depth_image.squeeze(0).to('cpu').detach().numpy().copy()
            print(depth.shape)

            depth = Image.fromarray((depth).astype(np.uint8))


            color_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/dataset_variety/{l}_{step}_color.png"
            depth_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/dataset_variety/{l}_{step}_depth.png"
        ...
        if position == 1 and step == 180:
            # print("yes")
            SAMPLE_DIR = "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/generator_1214"
            ...
            test_input_name = f"/current.pt"
            test_input_path = SAMPLE_DIR + test_input_name
            target_input_name = f"/target.pt"
...
if __name__ == "__main__": 
    ...
    step = random.sample(range(1, 180), 4)
    step.append(181)

    for step_number in range(5):
        rend.pre_physics_step(step_number, 0)

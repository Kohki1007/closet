...
class rendar():
    def __init__(self, num_envs = 1):
        ...
        self.ur_default_dof_pos = self.to_torch([-1.57, -1.2, -1.2, -0.9, 1.57, 0], device=self.device)
        self.ur_default_dof_pos = self.to_torch([0.05, -1.8, 1.2, -2.6, -1.57, 0], device=self.device)
        ...
        self.repeat = 0
        self.number =  86 + self.repeat * self.num_envs 
        ...
        self.load_weight("/home/engawa/py_ws/visual_servo/src/network/sim2real/log/generator_many_1214/weight/weights_last.pth")
        # self.model.to("cuda")
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        self.cam_pos = gymapi.Vec3(1.5, -0., 1.)
        self.cam_target = gymapi.Vec3(-0.8, 0, 1.0)
        ...
        asset_box = self.gym.create_box(self.sim, 0.8, 100, 5, asset_options)
    ...
    def set_ur_position(self, i):
        ...
        # self.root_tensor[:, 0, 0] = self.closet_position[self.number:(self.number + self.num_envs), i, 0] + 0.7
        # self.root_tensor[:, 0, 1] = self.closet_position[self.number:(self.number + self.num_envs), i, 1] - 0.05
    ...
    # def set_ur_position_first(position):
    ...
    #     self.gym.render_all_camera_sensors(self.sim)
    #     self.root_tensor[:, 0, 0] = -0.3
    #     self.root_tensor[:, 0, 0] = 0
    #     self.root_tensor[:, 0, 1] = -0.05
    ...
    def set_ur(self):
        ...
        # print(hand_pos)
        # print(self.rigid_body_states[:, 0:3])
        # print(ee_pos)
        ...
        self.gym.write_viewer_image_to_file(self.viewer, "/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/result/test.png")
    ...
    def step_ur(self, current_position, goal_position, i):
        ...
        while difference > 0.001:
            ...
            self.ur_dof_targets[:self.test_num_envs, :self.num_ur_dofs] += d_dof[:, 2:]
            
            self.root_tensor[:, 0, :2] += d_dof[:, :2]
    ...
    def ik(self, jacobian_end_effector: torch.Tensor,
        ...
        # print(jacobian_end_effector.shape)
    ...
    def pre_physics_step_brank(self):
        ...
        self.gym.write_viewer_image_to_file(self.viewer, f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/closet/0.png")
    ...
    def get_image(self, position, step):
        ...
        for l in range(self.num_envs):
            ...
            color_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/collor_generator.png"
            depth_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/predict_generator_{step}.png"
            input_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/si/target_generator.png"

            if step == 0 or step == 100:
                # image.save(color_path)
                with torch.no_grad():
                    # print(color_image_left)
                    test_depth = self.model(self.transform(color_image_left.unsqueeze(0)/255).repeat(1, 3, 1, 1).to("cuda")).squeeze()
                test_depth = test_depth.to('cpu').detach().numpy().copy()
                test_depth *= 255
                # color_image = color_image.permute(2, 0, 1)
        ...
        #     current_name = f"/input_ob_{self.repeat}.pt"
        #     # current_name = f"/input_st_{self.repeat}.pt"
        #     current_name = f"/input_handle.pt"
        #     # current_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/remover_ur/output_{self.repeat}.pt"
...
if __name__ == "__main__": 
    ...
    for position in range(5):
        ...
        rend.get_image(0, 100)
        rend.set_ur_position(position)
        # rend.set_ur_position_first(position)
        for step in range(40):
            rend.pre_physics_step(step, position)
            rend.set_ur()
            rend.get_image(position, step)

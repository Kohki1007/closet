...
parser.add_argument("--lrt", type=float, default=0.0002, help="学習率")
parser.add_argument("--epoch", type=str, default=300000000000, help="エポック数")
parser.add_argument("--batch", type=str, default=16, help="バッチサイズ")
parser.add_argument("--weight_path", type=str, default="log/trick_decoder_cnn/weight/weights_last.pth", help="重みのパス")
# parser.add_argument("--weight_path", type=str, default=None, help="重みのパス")
parser.add_argument("--save_path", type=str, default="trick_decoder_cnn_2", help="保存先のパス")
parser.add_argument("--current_path", type=str, default="trick/target_train.pt", help="入力１") ####### image1
parser.add_argument("--target_path", type=str, default="trick/current_train.pt", help="入力１")
parser.add_argument("--rot_path", type=str, default="trick/rot_train.pt", help="入力１")
parser.add_argument("--action_path", type=str, default=None, help="アクション")

parser.add_argument("--eval", type=str, default=True, help="入力１")
parser.add_argument("--current_path_eval", type=str, default="trick/target_test.pt", help="入力１") ####### image1
parser.add_argument("--target_path_eval", type=str, default="trick/current_test.pt", help="入力１")
parser.add_argument("--rot_path_eval", type=str, default="trick/rot_test.pt", help="入力１")
...
train_dataset = train_base.make_dataset(args.current_path, args.target_path, args.rot_path, args.action_path, args.batch)

if args.eval:
    # print(args.eval)
    eval_dataset = train_base.make_dataset_eval(args.current_path_eval, args.target_path_eval, args.rot_path_eval, args.action_path_eval)
    print(args.eval)
...
for epoch in range(args.epoch):
    ...
    with tqdm(enumerate(train_dataset),
        ...
        for i, (current, target, rot) in pbar_loss:
            loss, accuracy, lerning_rate = train_base.train_step(current, target, rot, epoch)
    ...
    if args.eval:
        ...
        with tqdm(enumerate(train_dataset),
              total=len(train_dataset)) as pbar_loss:
            for j, (current, target, rot) in pbar_loss:
                val_loss, val_accuracy, val_lerning_rate = train_base.eval_step(current, target, rot, epoch)

                val_all_loss += val_loss  
                val_accuracy_1 += val_accuracy[0]  
                val_accuracy_5 += val_accuracy[1]  

        print(val_lerning_rate)
        val_all_loss /= (j+1) 
        val_accuracy_1 /= (j+1) 
        val_accuracy_5 /= (j+1) 
    ...
    if (epoch+1)%20 == 0 :
        # if (epoch+1) < 210:
        #     train_base.modify_oprimazer_decoder(epoch)
        # elif (epoch+1) == 200:
        #     train_base.reset_optimizer()
        # else:
            # train_base.modify_oprimazer(epoch)

        train_base.modify_oprimazer_decoder()
    ...
    f.write(f'epoch {epoch} loss {train_loss} train_accuracy_1: {train_accuracy_1:.5f}, train_accuracy_5: {train_accuracy_5:.5f} val_loss {val_all_loss} val_accuracy_1: {val_accuracy_1:.5f}, val_accuracy_5: {val_accuracy_5:.5f}\n')
    ...
    f' lrt : {lerning_rate} train_loss: {train_loss:.5f}'\
    f' train_accuracy_1: {train_accuracy_1:.5f}, train_accuracy_5:: {train_accuracy_5:.5f}')

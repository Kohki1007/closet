...
class train_function():
    def __init__(self, train_subject = "goal_image_generator", 
        ...
        ################## model の指定         
        self.model = action_generator()
        self.model = manipulator_remover()
        self.model = goal_image_generator()
        # self.model = ivis()
        ...
        self.accuracy_list = np.zeros([9])

    def forward(self, image1, rot, image2 = None):
        image1 = image1.to(self.device)
        rot = rot.to(self.device)
        # image2 = image2.to(self.device)
        if self.train_subject == "goal_image_generator":
            # self.x = self.model(image1)
            self.x = self.model(image1, rot)
        elif self.train_subject == "manipulator_remover":
            self.x = self.model(image1)
        elif self.train_subject == "action_generator":
    ...
    def rename_key(self, d):
        ...
        for key in keys_list:
            new_key = key.replace('manipulator_remover.', '')
            d[new_key] = d.pop(key)
        return d
    
    def delete_key(self, weight):
        keys_to_delete = [key for key in weight.keys() if 'goal_image_generator' in key]
    ...
    def train_step(self, input1, output, rot, epoch, input2 = None):
        # if epoch > 100:
        ...
        self.forward(input1, rot, input2)
        ...
        return loss, accuracy, self.lerning_rate

    def eval_step(self, input1, output, rot, epoch, input2 = None):
    ...
    def train_step_remover(self, input1, output, epoch, noise_level, input2 = None):
        if epoch > 40:
    ...
    def eval_step_remover(self, input1, output, epoch, noise_level, input2 = None):
        if epoch > 40:
    ...
    def make_dataset(self, current_image, target_image, rot, action, batch = 4, eval = None):
        ...
        if eval == True:
            self.save_id_eval = [345, 678, 1023, 1987]
        # else:
        ...
        if self.train_subject == 'goal_image_generator' or self.train_subject == "manipulator_remover":
            ...
            self.rot = torch.load(self.dataset_base + rot).to(torch.float32)
            # print(self.rot)
            dataset = TensorDataset(self.input_1, self.output, self.rot)
    ...
    def make_dataset_eval(self, current_image, target_image, rot, action, batch = 4, eval = None):
        # print(self.dataset_base)
        # input
        self.eval = eval
        ...
        self.save_id_eval = [345, 678, 1023, 980]
        ...
        if self.train_subject == 'goal_image_generator' or self.train_subject == "manipulator_remover":
            ...
            self.rot_eval = torch.load(self.dataset_base + rot).to(torch.float32)
            dataset = TensorDataset(self.input_1_eval, self.output_eval, self.rot_eval)
        else :
    ...
    def modify_oprimazer_decoder(self):
        if self.lerning_rate > 0.000001:
            self.lerning_rate /= 1.3
            self.optimizer = optim.AdamW(           
                self.model.decoder.parameters(),
    ...
    def make_dir(self, path):
        ...
        if not os.path.exists(directory_path):
            ...
            os.makedirs(directory_path + "/image/980")
            os.makedirs(directory_path + "/image/980/depth")
            os.makedirs(directory_path + "/image/980/input_image")
    ...
    def save_sample_image(self, train_subject, path, epoch, eval = None):
        ...
        if eval:
            if epoch == 0:
                output_number = 0
                depth_set = self.transform_256(self.input_1_eval[self.save_id_eval])
                for i in range(len(self.save_id_eval)):
                    ...
                    depth = self.output_eval[self.save_id_eval][i, 0].to('cpu').detach().numpy().copy()
                    depth *= 255
                    depth = Image.fromarray((depth).astype(np.uint8))
    ...
    def forward_solo(self, input, output, step):
        ...
        for i in range(self.x.shape[0]):
            ...
            depth = Image.fromarray((depth).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth.save(f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/eval/remover_ur_sim/input/{step}_{i}.png")
            depth_predict = self.x[i, 0].to('cpu').detach().numpy().copy()
            depth_predict *= 255
            print(depth_predict.shape)
            depth_predict = Image.fromarray((depth_predict).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth_predict.save(f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/eval/remover_ur_sim/predict/{step}_{i}.png")
            depth = output[i, 0].to('cpu').detach().numpy().copy()
            depth *= 255
            depth = Image.fromarray((depth).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth.save(f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/eval/remover_ur_sim/output/{step}_{i}.png")
    ...
    def forward_trick_eval(self, image, rot):
        rot_tensor = torch.tensor(rot)
        rot_tensor = rot_tensor.repeat(image.shape[0]).unsqueeze(1)
        image = self.transform(image)
        self.forward(image, rot_tensor)
        for i in range(self.x.shape[0]):
            depth = image[i, 0].to('cpu').detach().numpy().copy()
            depth *= 255
            depth = Image.fromarray((depth).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth.save(f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/eval/trick_sim/input/{i}.png")
            depth_predict = self.x[i, 0].to('cpu').detach().numpy().copy()
            depth_predict *= 255
            # print(depth_predict.shape)
            depth_predict = Image.fromarray((depth_predict).astype(np.uint8))
            # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            depth_predict.save(f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/eval/trick_sim/predict/{i}_{rot}.png")
            # depth = output[i, 0].to('cpu').detach().numpy().copy()
            # depth *= 255
            # depth = Image.fromarray((depth).astype(np.uint8))
            # # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
            # depth.save(f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/eval/trick/output/{i}_{rot}.png")
            # depth_predict = target[i, 0].to('cpu').detach().numpy().copy()
            # depth_predict *= 255
            # depth_predict = Image.fromarray((depth_predict).astype(np.uint8))
            # # depth_predict.save(f"image/sice/{sample_id[i]}/{epoch}.png")
    ...
    def eval_generator(self, input, output, number):
        input = input.squeeze().unsqueeze(1)
        input = input.repeat([1, 3, 1, 1])
        self.forward(input)
        accuracy_1, accuracy_5 = self.accuracy(self.x, output)
        id = number//20

        self.accuracy_list[id] += accuracy_1

    def draw_accuracy_list(self):
        self.accuracy_list /= 20
        plt.hist(self.accuracy_list, bins=9, edgecolor='black', range=(0, 9))
        # plt.xlabel('Value')

...
class rendar():
    def __init__(self, num_envs = 10):
        ...
        self.current_image = torch.zeros([self.num_envs, 5, 180, 224, 224])
        self.target_image = torch.zeros([self.num_envs, 5, 180, 256, 256])

        self.closet_position = torch.zeros(self.num_envs, 5, 2)
        # self.closet_rot = torch.zeros(self.num_envs, 5, 41)
        ...
        for i in range(self.closet_position.shape[0]):
            ...
            for j in range(self.closet_position.shape[1]):
                ...
                self.closet_position[i, j, 1] = random.uniform(0.1 - dy_min, dy_max - 0.1)
            #     self.closet_rot[i, j, :-1] = torch.tensor(sorted(random.sample(range(170), k=40)))
    ...
    def create_envs(self, i = 1, spacing = 2, num_per_row = 5):
        ...
        for a in range(self.num_envs):
            ...
            handle_position = self.closet_size[a, 1] 
            scale_id = self.closet_size[a, 0]

            edit_urdf_real(scale_id, a, handle_position)
        ...
        for i in range(self.num_envs):
            ...
            closet_pose.p.x = self.closet_position[asset_id , 0, 0]
            closet_pose.p.y = self.closet_position[asset_id , 0, 1]
            closet_pose.p.z = 0
            ...
            pose.p = gymapi.Vec3(self.wall_position[i], 0, 1.25)
    ...
    def set_closet_position(self, position):
        ...
        self.root_tensor[:, 1, :2] = self.closet_position[:, position]
    ...
    def pre_physics_all_image(self, step, position):
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self.gym.refresh_dof_state_tensor(self.sim)
        self.gym.refresh_rigid_body_state_tensor(self.sim)
        self.gym.refresh_jacobian_tensors(self.sim)
    ...
    def get_image(self, position, step):
        ...
        for l in range(self.num_envs):
            ...
            if step == 180:
                ...
                self.target_image[l, position, :] = self.transform_256(depth_image).clone()
            else:
                # print(depth_image.shape)
                # print(self.transform(depth_image).shape)
                self.current_image[l, position, step] = self.transform(depth_image).clone()

            # color_path = f"/home/engawa/py_ws/visual_servo/src/network/sim2real/image/target_ob/{l}_{position}.png"
        ...
        if position == 4 and step == 180:
            SAMPLE_DIR = "/home/engawa/py_ws/visual_servo/src/network/sim2real/dataset/eval/generator_accuracy"
            ...
            train_input_name = f"/target_{self.repeat}.pt"
            train_input_path = SAMPLE_DIR + train_input_name
            test_input_name = f"/current_{self.repeat}.pt"
            test_input_path = SAMPLE_DIR + test_input_name

            
            torch.save(self.target_image, train_input_path)
            torch.save(self.current_image, test_input_path)
            print("complete")
...
if __name__ == "__main__": 
    ...
    for position in range(5):
        ...
        for step in range(181):
            rend.pre_physics_all_image(step, position)
